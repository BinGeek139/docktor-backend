package com.techasians.doctor.management.web.rest;
...
import com.techasians.doctor.management.config.KafkaProperties;
...
class ManagementKafkaResourceIT {
    ...
    void setup() {
        ...
        ManagementKafkaResource kafkaResource = new ManagementKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/management-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/management-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
